...
ALTER TABLE Review DISABLE TRIGGER after_insert_review;
ALTER TABLE Review DISABLE TRIGGER after_review_insert;
ALTER TABLE Booking DISABLE TRIGGER after_booking_insert;
ALTER TABLE Booking DISABLE TRIGGER before_booking_insert;
...
ALTER TABLE Review ENABLE TRIGGER after_insert_review;
ALTER TABLE Review ENABLE TRIGGER after_review_insert;
ALTER TABLE Booking ENABLE TRIGGER after_booking_insert;
ALTER TABLE Booking ENABLE TRIGGER before_booking_insert;
...
WITH HostRatings AS (
    ...
    FROM
        Host H
        LEFT JOIN Property P ON H.user_id = P.host_user_id
        LEFT JOIN Booking B ON P.property_id = B.property_id
        LEFT JOIN Review R ON B.booking_id = R.booking_id
    GROUP BY H.user_id
)
...
FROM (
    ...
    FROM
        Booking B
        LEFT JOIN Property Pr ON B.property_id = Pr.property_id
        LEFT JOIN Promotion P ON Pr.property_id = P.property_id
        LEFT JOIN Guest G ON B.guest_user_id = G.user_id
) AS subquery
...
WHERE user_id IN (
    ...
    WHERE
        Guest.points > 29 AND B.timestamp BETWEEN P.start_date AND P.end_date
